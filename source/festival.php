...
class festival
{
    ...
    {
        // $this->loadDictionary();
        // assertTrue(count($this->dictionary) > 0, 'Nothing loaded in Dictionary');
    ...
    //     assertTRUE(count($this->searchOnly('abandon', '', '')) == 1);
    //     assertTRUE(count($this->searchOnly('aband*n', '', '')) == 1);
    //     assertTRUE(count($this->searchOnly('abandon*', '', '')) == 6);  // abandon, abandonment, etc
    //     assertTRUE(count($this->searchOnly('abandon*', '', '', 3)) == 3);  // does MAX work?

    //     // check for phoneme
    //     assertTRUE(count($this->searchOnly('abandon*', 'ng', '')) == 1); // abandoning

    //     // check for spelling
    //     assertTRUE(count($this->searchOnly('abandon*', '', 'ng;ng')) == 1); // abandoning
    //     return (true);
    // }
...
*/
    ...
    {  // spell;phone pairs with ./- separators intact
        ...
        while (!empty($aPlausible)) {
            ...
            if (!isset($this->phoneSet[$left_ph])) assertTRUE(false, "phoneSet[$left_ph] is not set in $word");

            foreach ($this->phoneSet[$left_ph]['spellings'] as $spelling) {

                if (!isset($this->phoneSet[$left_ph]['spellings'])) assertTRUE(false, "phoneSet[$left_ph] is not set in $word");
    ...
    function parseEntry($entry)
    {
        ...
        while ($tokCurrent < strlen($entry)) {
            ...
            switch ($state) {
                case 'idle':
                    //assertTRUE($token=='(',"Expecting '(' in state $state parsing $entry");
                    $state = 'startreadword';
                    break;
                case 'startreadword':
                    //assertTRUE($token=='"',"Expecting '\"' in state $state parsing $entry");
    ...
    /*function testWordisValid(){
        if(!$this->shortDict){    // may not be in dict if we are using shortDict
            assertTRUE($this->wordIsValid('hello'),'Should be OK');
            assertTRUE(!$this->wordIsValid('helloxxx'),'Should NOT be OK');
            assertTRUE($this->wordIsValid('looking'),'Has an affix');
        }
        return(true);
    }
    */
            ...
            foreach (explode(',', $wordList) as $word) {
                ...
                if (strpos($word, '/') > 0) {    // check for can/cad  pairs
                    foreach (explode('/', $word) as $subword) {
                        assertTRUE($this->wordIsValid(ltrim($subword)), "Invalid '$subword' in $wordList, rejected by dictionary");
                    }
                } else {
                    if (strpos($pdbqWords, $word) === false) {
                        if (strpos($ignore, $word) === false)    // skip the pbdq and ignore words

                            assertTRUE($this->wordIsValid(ltrim($word)), "Invalid '$word' in $wordList");
                    }
                }
            }
        }
    ...
    function xtestCMUgets()
    {
        unset($GLOBALS['CMUcount']);

        $test = $this->CMUfgets(2);
        printNice($test);
        $test = $this->CMUfgets(2);
        printNice($test);
        $test = $this->CMUfgets(2);
    ...
    {
        if ($n > 0) {
            if (!isset($GLOBALS['CMUcount'])) {
                printNice("WARNING - Short Dictionary - only $n records.");
                $GLOBALS['CMUcount'] = 0;
            }
            if ($GLOBALS['CMUcount']++ >= $n) {
                if (($entry = fgets($this->fileHandle)) === false) {   // close up if we get to the end
    ...
    function generateDictionary($n = 0)
    {
        ...
        printNice("about to generate the dictionary (zzzz..)");
        $startTime = time();
        ...
        while ($candidate = $this->CMUfgets($n)) {
            ...
            if (empty($p2s['failPhone'])) {
                $this->dictionary[$p2s['word']] = $p2s['spelling'];
            } else {
        ...
        $cwd = getcwd();
        $prefix = "<?" ."php\n\n";
        $prefix .= "// This file was created mechanically from the CMU spelling dictionary\n\n";
        ...
        file_put_contents($cwd . '/source/dictionaryFailed.php',
